 ...
 *
 * This encapsulates all Supabase and query logic so the route handler
 * can stay small and focused on auth and wiring.
 */
export async function handleGetAnnouncements(
  ...
  if (isGuardian && guardianClassIds.length > 0) {
    ...
    finalTeacherIds = teacherIds.filter((idVal) => !principalIdsSet.has(idVal));
    ...
    const teacherClassIdsArr = teacherClassIdsCsv
      ? teacherClassIdsCsv.split(',').map((s) => s.trim()).filter(Boolean)
      : classId
        ? [classId]
        : [];

    if (teacherClassIdsArr.length > 0) {
      ...
      query = query.or(
        `class_id.is.null,class_id.in.(${teacherClassIdsArr.join(',')})`
      );
    } else if (classId) {
  ...
  if (!error && data && isGuardian) {
    ...
    data = data.filter((ann: any) => {
      ...
      if (guardianClassIds.length > 0 && ann.class_id) {
        if (
          guardianClassIds.includes(ann.class_id) &&
          finalTeacherIds.length > 0 &&
          finalTeacherIds.includes(ann.author_id)
        ) {
          return true;
        }
      }
    ...
    if (guardianClassIds.length === 0) {
      ...
      data = data.filter((ann: any) => !ann.class_id);
    }
  ...
  const transformedData: Announcement[] = (data || []).map((ann: any) => {
    const classData =
      Array.isArray(ann.classes) && ann.classes.length > 0
        ? ann.classes[0]
        : ann.classes && !Array.isArray(ann.classes)
          ? ann.classes
          : null;
