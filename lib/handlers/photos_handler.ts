  ...
  const photosWithUrls = await Promise.all(
    photos.map(async (photo: any) => {
      ...
      return {
        ...
        students: photo.student_id
          ? studentsMap.get(photo.student_id) || null
          : null,
      };
    }),
  );
...
export async function handlePostPhotos(
  request: Request,
  user: AuthUser,
  adminClient: SupabaseClient,
) {
  ...
  for (const file of files) {
    try {
      ...
      const { data: uploadData, error: uploadError } =
        await adminClient.storage.from('photos').upload(filePath, buffer, {
      ...
      const { data: uploadRecord, error: uploadRecordError } =
        await adminClient
          .from('uploads')
          .insert({
            org_id: orgId,
      ...
      if (uploadRecordError || !uploadRecord) {
        ...
        await adminClient.storage.from('photos').remove([filePath]);
        throw new Error(`Failed to create upload record for ${file.name}`);
      }

      // Create photos record
      const { data: photoRecord, error: photoRecordError } =
        await adminClient
          .from('photos')
          .insert({
            org_id: orgId,
      ...
      if (photoRecordError || !photoRecord) {
        ...
        await adminClient.storage.from('photos').remove([filePath]);
        await adminClient.from('uploads').delete().eq('id', uploadRecord.id);
...
export async function handleDeletePhotos(
  request: Request,
  user: AuthUser,
  adminClient: SupabaseClient,
) {
  const { searchParams } = new URL(request.url);
  const queryValidation = validateQuery(deletePhotosQuerySchema, searchParams);
  if (!queryValidation.success) {
    return queryValidation.error;
  }

  const { photoId } = queryValidation.data;
  ...
  // if (upload?.path) {
  //   await adminClient.storage.from('photos').remove([upload.path]);
  // }
