...
export async function handleCreateAnnouncement(
  ...
  try {
    const { data: existing } = await adminClient
      ...
      .maybeSingle();
    if (!existing) {
      const { error: upErr } = await adminClient.from('users').upsert(upsertPayload, { onConflict: 'id' });
      if (upErr) {
        ...
        const { error: insErr } = await adminClient.from('users').insert(minimal).select('id').maybeSingle();
        if (insErr) {
  ...
  let { data: announcement, error } = await adminClient
    .from('announcements')
  ...
  if (error && /org_id/i.test(error.message)) {
    ...
    ({ data: announcement, error } = await adminClient
      .from('announcements')
      .insert(retryPayload)
      .select()
      .single());
...
export async function handleSwitchUserRole(
  user: AuthUser,
  adminClient: SupabaseClient,
  newRole: SamveraRole
) {
...
export async function handleGetClassData(
  user: AuthUser,
  adminClient: SupabaseClient,
  classId: string
) {
...
export async function handleGetOrgUsers(
  user: AuthUser,
  adminClient: SupabaseClient,
  orgId: string
) {
...
export async function handleGetPaginatedNotifications(
  user: AuthUser,
  adminClient: SupabaseClient,
  page: number = 1,
  limit: number = 10
