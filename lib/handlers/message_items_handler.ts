...
export async function handlePutMessageItem(
  request: Request,
  user: AuthUser,
  adminClient: SupabaseClient,
) {
  ...
  // Update edit history
  const editHistory = Array.isArray(existingItem.edit_history)
    ? existingItem.edit_history
    : [];
  editHistory.push({
...
export async function handleDeleteMessageItem(
  request: Request,
  user: AuthUser,
  adminClient: SupabaseClient,
) {
