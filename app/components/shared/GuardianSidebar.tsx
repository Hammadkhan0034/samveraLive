...
import { LayoutDashboard, MessageSquare, Camera, CalendarDays, Utensils, ClipboardCheck, Baby, Megaphone } from 'lucide-react';
...
export interface GuardianSidebarTile {
  id: string;
  title: string;
  desc: string;
...
export interface GuardianSidebarProps {
  ...
  tiles?: GuardianSidebarTile[]; // For any custom tiles
}

export interface GuardianSidebarRef {
  open: () => void;
  close: () => void;
}
...
const builtInTileRoutes: BuiltInTileConfig[] = [
  { id: 'dashboard', route: '/dashboard/guardian' },
  { id: 'messages', route: '/dashboard/guardian/messages' },
  { id: 'media', route: '/dashboard/guardian/media' },
  { id: 'calendar', route: '/dashboard/guardian/calendar' },
  { id: 'announcements', route: '/dashboard/guardian/announcements' },
...
const GuardianSidebarContent = forwardRef<GuardianSidebarRef, GuardianSidebarProps>(({
  messagesBadge,
  tiles = [],
}, ref) => {
  ...
  const handleCustomTileClick = (tile: GuardianSidebarTile) => {
          ...
          <nav className="space-y-3">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.messages_desc || 'View and send messages'}</p>
              </div>
            </button>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <span className={clsx(
                    ...
                    {t.tile_media || t.media || 'Media'}
                  </span>
                </div>
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.tile_media_desc || t.media_desc || 'View photos and albums'}</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.calendar_desc || 'View calendar events'}</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.announcements_subtitle || 'View school announcements'}</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.menu_desc || 'View daily menus'}</p>
              </div>
            </button>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate mt-0.5">{t.attendance_desc || 'View attendance records'}</p>
              </div>
            </button>
...
GuardianSidebarContent.displayName = 'GuardianSidebarContent';

const GuardianSidebar = forwardRef<GuardianSidebarRef, GuardianSidebarProps>((props, ref) => {
  return (
    <Suspense fallback={<div className="w-[280px] bg-white dark:bg-slate-800 rounded-tr-[24px] rounded-br-[24px]" />}>
      <GuardianSidebarContent {...props} ref={ref} />
    </Suspense>
  );
});
