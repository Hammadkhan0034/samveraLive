...
function GuardianDashboardPageContent() {
  const { t } = useLanguage();
  ...
  const fetchMetrics = useCallback(async (signal: AbortSignal) => {
    try {
      ...
      const res = await fetch(`/api/guardian-dashboard-metrics?t=${Date.now()}`, {
        cache: 'no-store',
        signal,
        credentials: 'include',
      });
      ...
      setLinkedStudentsCount(data.linkedStudentsCount || 0);
      setMessagesCount(data.messagesCount || 0);
      setPhotosCount(data.photosCount || 0);
      setHealthLogsCount(data.healthLogsCount || 0);
    } catch (err: unknown) {
      if (signal.aborted) {
        return;
      }
  ...
  const kpis = useMemo<KPICard[]>(() => [
    {
      label: t.tile_students || 'Linked Students',
    ...
    {
      label: t.health_logs || 'Health Logs',
      value: healthLogsCount,
      icon: icons.Baby,
    },
  ], [t, linkedStudentsCount, messagesCount, photosCount, healthLogsCount, icons]);

  return (
    <GuardianPageLayout messagesBadge={messagesCount > 0 ? messagesCount : undefined}>
